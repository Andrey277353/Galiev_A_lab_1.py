"""
Generic lab script: trains SVM and Logistic Regression on a binary target derived
from a numeric column (default: 'mental_wellness_index_0_100' with threshold 15).
- Works with any CSV in the current folder (auto-detects if --csv not given).
- Handles messy headers (tabs, quotes, 'Unnamed:*').
- Robust CSV loading: tries tab, comma, auto sniffing.
Usage (PowerShell):
  python -m venv .venv
  .\.venv\Scripts\Activate.ps1
  python -m pip install --upgrade pip
  python -m pip install -r requirements_lab4.txt
  python lab4_generic.py                # auto-pick CSV in folder
  python lab4_generic.py --csv data.csv # explicit path
Options:
  --target TARGET            Numeric column name for binarization (default: mental_wellness_index_0_100)
  --threshold FLOAT          Threshold to binarize target (>= threshold => 1)
  --test-size FLOAT          Test size for train/test split (default: 0.25)
  --random-state INT         Random seed (default: 42)
  --id-cols COL [COL ...]    Columns to drop as identifiers (default: user_id id index)
  --save                     Save results to results.txt and results.json
"""
